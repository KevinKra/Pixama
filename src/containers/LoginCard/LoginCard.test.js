...
describe('LoginCard', ()=> {
  ...
  describe('mapDispatchToProps', ()=>{
    ...
    it('calls dispatch with a getFavorites action when onSubmit is called',()=>{
      const mockDispatch = jest.fn()
      const actionToDispatch = getFavorites([{title: "Fight Club"}])
      const mappedProps = mapDispatchToProps(mockDispatch)
      mappedProps.getFavorites([{title: "Fight Club"}])
      expect(mockDispatch).toHaveBeenCalledWith(actionToDispatch)
    });
